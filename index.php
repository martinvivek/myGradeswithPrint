...
/*Duilio modifico para incluir js*/
$PAGE->requires->js('/report/mygrades/init.js', true);
$PAGE->requires->js('/report/mygrades/imprimirReporte.js', true);
require_login();
...
class report_mygrades extends block_base {
	...
	//funcion que calcula la fecha, en el formato necesario, en base a como retorna la fecha moodle
	public function calcularFecha($moodleDate){
				$anio= $moodleDate['year'];
				$mes= date('m',strtotime($moodleDate['month']));
				//echo "</br></br>".$mes. "</br></br>";
				$dia = $moodleDate['yday'];
				//calculo el dia en base a yday(dia en el año).
				$mesAnterior =1;
				while($dia > days_in_month($mes, $anio)){
					$dia = $dia -days_in_month($mesAnterior,$anio);
					$mesAnterior++;
				}				
				return $dia ."/".$mes."/".$anio;
	...
	public function get_content() {
		...
		if (is_array($newdata))
		{
			if (count($newdata)>0)
			{
				$newtext="<table class=\"grades\" id=\"grades\"><thead><tr><th>".get_string('gradetblheader_course', 'report_mygrades')."</th><th>".get_string('gradetblheader_grade', 'report_mygrades')."</th><th>".get_string('gradetblheader_startdate', 'report_mygrades')."</th></tr></thead>";
				foreach($newdata as $newgrade)
				{
					// need to put data into table for display here
					$newtext.="<tr class='filas'><td>{$newgrade[0]}</td><td>{$newgrade[1]}</td><td>{$newgrade[2]}</td></tr>";
				}
				$newtext.="</table>";
	...
	public function grade_data($report) {
		...
		if ($courses = enrol_get_users_courses($report->user->id, false, 'id, shortname, showgrades')) {
			...
			foreach ($courses as $course) {
				...
				//Duilio modificacion para mostrar la fecha de inicio del curso				
				$fechaaux = usergetdate($course->startdate);
				$fecha = $this->calcularFecha($fechaaux);					
				...
				$data[] = array($courselink, grade_format_gradevalue($finalgrade, $course_item, true),$fecha);
			}
...
//error_log("mostrar tabla resultado final".$content->text);
echo $content->text;
echo "</br></br><form id='form_imprimir' method='POST' action='".$CFG->wwwroot."/report/mygrades/imprimirReporte.php'>"."<input type='submit' value='".get_string('reportePDF', 'report_mygrades')."'></form>";
